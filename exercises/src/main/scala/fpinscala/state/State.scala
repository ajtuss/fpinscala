...
object RNG {
  ...
  def sequence[A](fs: List[Rand[A]]): Rand[List[A]] = fs.foldRight(unit(List()))((f, acc) => map2(f, acc)(_ :: _))

  def _ints(count: Int)(rng: RNG): Rand[List[Int]] = sequence(List.fill(count)(int))
