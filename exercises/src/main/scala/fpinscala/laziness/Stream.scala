...
trait Stream[+A] {
  ...
  def startsWith[B](s: Stream[B]): Boolean = zipAll(s).takeWhile(_._2.isDefined).forAll { case (h, h2) => h == h2 }
...
object Stream {
  ...
  def fibs: Stream[Int] = {
    def fib(n0: Int, n1: Int): Stream[Int] = cons(n0, fib(n1, n0 + n1))
  ...
  def fibsViaUnfold: Stream[Int] = unfold((0, 1)) { case (n0, n1) => Some(n0, (n1, n0 + n1)) }
